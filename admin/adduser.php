...
if (!isPasswordStrong($passwords)) {
	...
	if(file_exists($_FILES['resume']['tmp_name'])) { 
		...
		if($resumeFileType == "pdf" or $resumeFileType == "docx")  {
			...
			if($_FILES['resume']['size'] < 1000000) { // File size is less than 10MB
	...
	if($uploadOk == false) {
		...
		$sql = "INSERT INTO users(firstname, lastname, email, password, address, city, state, contactno, qualification, stream, passingyear, dob, age, designation, resume, hash, aboutme, skills,active,creationDate,gender) VALUES ('$firstname', '$lastname', '$email', '$password', '$address', '$city', '$state', '$contactno', '$qualification', '$stream', '$passingyear', '$dob', '$age', '$designation', '$file', '$hash', '$aboutme', '$skills',0,'$today','$gender')";
